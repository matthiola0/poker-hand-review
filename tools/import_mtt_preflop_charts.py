"""把 gto-preflop/charts/ 的 MTT 範圍轉成引擎格式，寫入 src/.../gto/charts/。

來源（本機，未上傳）：gto-preflop/charts/<stack>bb/{rfi,vs-open}/<name>.json
...
- 截圖位置 LJ -> 引擎 MP（引擎 8-max 第三位叫 MP，無 LJ）
...
SRC_CHARTS = ROOT / "gto-preflop" / "charts"
...
def meta_for(bucket: str, stack: int, scenario: str, name: str, chart_id: str) -> dict:
    return {
        "source": "n8 MTT preflop ranges (screenshots)",
        "version": "1.0",
        ...
        "stack_bucket": bucket,
        "chart_id": chart_id,
        "notes": "Imported from personal MTT range screenshots; actions=四動作頻率, freqs=1-fold。",
    }
